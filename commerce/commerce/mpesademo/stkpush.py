...
def initiate_stk_push(request):
    ...
    if isinstance(access_token_response, JsonResponse):
        ...
        if access_token:
            ...
            business_short_code = '600996'
            process_request_url = 'https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest'
            callback_url = 'https://kariukijames.com/callback' # replace with your own url - the route to be executed when user clicks send
            timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
            password = base64.b64encode((business_short_code + passkey + timestamp).encode()).decode()
            party_a = business_short_code
            party_b = '254708374149' # i'll try and use my own cred/till/phone number
            account_reference = 'njoro'
